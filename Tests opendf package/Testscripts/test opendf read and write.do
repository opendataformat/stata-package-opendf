...
program define test_opendf_read_and_write
	local verbose = 1
	****************Test 1 : read and write data.zip *******************
	quietly {
	...
	****************Test 2 : read and write data_special_values.zip *******************
	quietly {
	...
	****************Test 3 : read and write data_with_default.zip *******************
	quietly {
		...
		// Unzip file first
		// Specify the path to the zip file and the destination folder
		local zipfile "`testdata_dir'\data_with_default.zip"
		cd "`output_dir'"
		unzipfile "`zipfile'"
		cd "`test_dir'"
		// Define location of extracted data.csv
		...
		*local original_csv = `"`macval(line)'"'
		...
		// Read and Write data.zip and read CSV from opendf write output as text
		// to check for any changes
		opendf read "`testdata_dir'/data_with_default.zip", clear
		capture confirm file "$output_folder\testdata"
		if (_rc != 601){
			noisily: di as error "Error in Test 3 opendf read: Zip-File created."
			local error_occured3="TRUE"
		}
		opendf write "Output Testscripts\data_with_default.zip", clear

		local zipfile "`output_dir'\data_with_default.zip"
		cd "`output_dir'"
		unzipfile "`zipfile'"
		cd "`test_dir'"
		...
		*local new_csv = `"`macval(line)'"'
		...
		if(`"`original_csv'"'!=`"`new_csv'"'){
			noisily: di as error "Error in Test 3 opendf read and write: CSV files are not equal after reading and writing with opendf read and opendf write with data.zip."
			local error_occured3="TRUE"
		}
		forvalues line = 3/`original_xml_linenum'{
			if ( subinstr(`"`original_xml_line`line''"', " ", "", .) != subinstr(`"`new_xml_line`line''"', " ", "", .)){
				local error_occured3="TRUE"
				noisily: di as error "Error in Test 3 opendf read and write: XML files are not equal in line `line' after reading and writing with opendf read and opendf write with data_special_values.zip."
				if $verbose == 1 {
					no di "Original line (`line'): "
					no di `"`original_xml_line`line''"'
					no di "New line (`line'): "
	...
	****************Test 4 : read and write data_with_missings.zip *******************
	quietly {
		...
		// Unzip file first
		// Specify the path to the zip file and the destination folder
		local zipfile "`testdata_dir'\data_with_missings.zip"
		cd "`output_dir'"
		unzipfile "`zipfile'"
		cd "`test_dir'"
		// Define location of extracted data.csv
		...
		*local original_csv = `"`macval(line)'"'
		...
		// Read and Write data.zip and read CSV from opendf write output as text
		// to check for any changes
		opendf read "`testdata_dir'/data_with_missings.zip", clear
		capture confirm file "$output_folder\testdata"
		if (_rc != 601){
			noisily: di as error "Error in Test 4 opendf read: Zip-File created."
			local error_occured4="TRUE"
		}
		opendf write "Output Testscripts\data_with_missings.zip", clear

		local zipfile "`output_dir'\data_with_missings.zip"
		cd "`output_dir'"
		unzipfile "`zipfile'"
		cd "`test_dir'"
		...
		*local new_csv = `"`macval(line)'"'
		...
		if(`"`original_csv'"'!=`"`new_csv'"'){
			noisily: di as error "Error in Test 4 opendf read and write: CSV files are not equal after reading and writing with opendf read and opendf write with data.zip."
			local error_occured4="TRUE"
		}
		forvalues line = 3/`original_xml_linenum'{
			if ( subinstr(`"`original_xml_line`line''"', " ", "", .) != subinstr(`"`new_xml_line`line''"', " ", "", .)){
				local error_occured4="TRUE"
				noisily: di as error "Error in Test 4 opendf read and write: XML files are not equal in line `line' after reading and writing with opendf read and opendf write with data_special_values.zip."
				if $verbose == 1 {
					no di "Original line (`line'): "
					no di `"`original_xml_line`line''"'
					no di "New line (`line'): "
	...
	if("`error_occured1'" =="FALSE" & "`error_occured2'" =="FALSE" & "`error_occured3'" =="FALSE" & "`error_occured4'" =="FALSE"){
